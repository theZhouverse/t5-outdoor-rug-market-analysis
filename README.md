...
> **当前生效规范**：`docs/SPEC.md` 2.0（2026-09-10）。本版本把业务交付固定为整体市场、PP市场、非PP市场、GENIMO品牌四部分；以原始卖家精灵工作簿为主源，先解析并标记小类BSR 1—100（包含100），再按标题中的完整单词 `plastic` 划分PP和非PP。当前 HTML 由 `src/build_spec2_html.mjs` 直接从原始主源生成；当前可回勾 XLSX 为 `outputs/20260911-spec2-market-analysis/户外地垫市场分析-SPEC2-可回勾版.xlsx`。2025.10—2026.07 子表均已从主源读取，读取器会自动识别“首行表头”和“首行导出说明、次行表头”两种结构；空值仍保留为空，不把缺失月份伪造为 0。
...
# 6. 一次运行旧版全量数据、分析和覆盖保护验收
npm test
```

### 3.1 领导验收结果工作簿

当前按 SPEC 2.0 生成的可回勾工作簿为 `outputs/20260911-spec2-market-analysis/户外地垫市场分析-SPEC2-可回勾版.xlsx`。工作表顺序为：`00_总览与校验`、`01_整体市场月度`、`02_PP市场月度`、`03_非PP市场月度`、`04_GENIMO品牌月度`、`05_年度同比`、`06_BSR分层`、`90_原始输入`、`91_去重明细`、`92_聚合输入`、`93_数据校验`、`94_规则说明`。所有展示指标的 SUMIFS、均价、成交均价、同比、环比和分层回勾公式均写在 01—06 页；90/91 页保留逐行输入和确定性去重明细，92 页提供可复核的聚合中间层。
...
工作表映射如下：`00_总览与结论` 导航、关键回勾和趋势判断；`01/02/03` 整体市场的月度年度、BSR前100、粗细分层；`04/05/06` PP管同样三张；`07/08/09` 非PP高客单同样三张；`10/11/12` GENIMO品牌份额、进退层、2027链接规划（只使用原始可用字段）；`13_周报汇总` 按参考周报表头组织领导可读的市场、分层和品牌摘要；`14_利润测试` 仅作为历史辅助页，按主源已有字段展示，不计算利润；`15_经营计划与实际` 复制计划部经营底表并追加 H1 达成、7 月差额和执行状态；`90_输入_明细`、`91_聚合输入`、`92_校验`、`93_来源与规则` 分别记录可追溯输入、中间聚合、数学校验和操作边界。
