...
> **当前生效规范**：`docs/SPEC.md` 2.0（2026-09-10）。本版本把业务交付固定为整体市场、PP市场、高客单价市场、GENIMO品牌四部分；以原始卖家精灵工作簿为主源，先解析并标记小类BSR 1—100（包含100），再按标题中的完整单词 `plastic` 划分PP和高客单价市场（高客单价市场为PP补集）。当前 HTML 由 `src/build_spec2_html.mjs` 直接从原始主源生成；当前可回勾 XLSX 为 `outputs/20260911-spec2-market-analysis/户外地垫市场分析-SPEC2-可回勾版.xlsx`。2025.10—2026.07 子表均已从主源读取，读取器会自动识别“首行表头”和“首行导出说明、次行表头”两种结构；空值仍保留为空，不把缺失月份伪造为 0。
...
- **输出**: `data/processed/market.db` (单文件 SQLite, ~48 MB)；当前可回勾工作簿 `outputs/20260911-spec2-market-analysis/户外地垫市场分析-SPEC2-可回勾版.xlsx`；旧版公式工作簿 `outputs/20260909-formula-market-analysis/户外地垫市场分析-公式版-20260909.xlsx` 仅作历史参考
...
- **历史实现记录（不作为 SPEC 2.0 新口径）**: 2026.01-06 竞品快照为**全市场父体级导出**（每月 1,038-1,993 个父体），2025 则为包含 ASIN/父ASIN 富文本标识的行级导出（含变体行）。两者统计单元不同，跨年数值只作方向性参考，不构成严格同口径同比；2026.07 的 94 父体小样本只展示规模，结果工作簿的 MOM/YOY留空。销量/销售额空值保留为缺失，已知值合计不把空值当业务零值；加权成交均价仅按同时具备销量和销售额的记录计算并提供覆盖率。BSR 支持数值、整数文本和 `N.0` 文本，非零小数按缺失处理；2026.01-07 缺少竞品名次增强时分析直接失败；并列名次保留，分层数量可能超过区间宽度。历史交付对所有月份统一采用父ASIN优先的确定性代表记录，90页保留原始行数和去重行数。竞品库可由 `src/build_competitor_db.js` 确定性重建。新流程以原始主源和 SPEC 2.0 为准。
...
│   ├── TASKS.md               # 任务清单（Phase 12 对齐任务进行中）
│   └── REVIEWS.md             # 审查记录和限制
├── data/
...
│   ├── build_spec2_html.mjs             # SPEC 2.0 原始主源 -> 四部分静态 HTML
│   ├── dump_spec2_dataset.mjs            # 导出同一 buildDataset 的 XLSX 中间 JSON
│   ├── build_spec2_xlsx.py               # SPEC 2.0 公式工作簿生成（含回勾公式）
...
│   └── 户外地垫市场分析-公式版-20260909.xlsx # 历史工作簿
├── outputs/20260911-spec2-market-analysis/
│   └── 户外地垫市场分析-SPEC2-可回勾版.xlsx # 当前四部分结果与公式
├── tmp/                       # 临时文件 (可随时清空)
...
# 3. 执行旧版转换（基础 Excel 导入 + 竞品库重建 + 2026.01-07 竞品代表行替换；SPEC 2.0 对齐完成前仅用于历史复现）
npm run import

# 4. 生成优化版结构化数据、完整 Markdown、极速版 Markdown 和独立 HTML
npm run analyze

# 5. 按 SPEC 2.0 从原始主源重建四部分 HTML（当前 HTML 交付）
node src/build_spec2_html.mjs

# 6. 生成可回勾 XLSX（先导出同一 buildDataset 的 JSON，再写入公式工作簿）
node src/dump_spec2_dataset.mjs
python src/build_spec2_xlsx.py

# 7. 一次运行旧版全量数据、分析和覆盖保护验收
npm test
```

### 3.1 领导验收结果工作簿

当前按 SPEC 2.0 生成的可回勾工作簿为 `outputs/20260911-spec2-market-analysis/户外地垫市场分析-SPEC2-可回勾版.xlsx`。工作表顺序为：`00_总览与校验`、`01_整体市场月度`、`02_PP市场月度`、`03_高客单价市场月度`、`04_GENIMO品牌月度`、`05_年度YOY`、`06_BSR分层`、`90_原始输入`、`91_去重明细`、`92_聚合输入`、`93_数据校验`、`94_规则说明`。所有展示指标的 SUMIFS、均价、加权成交均价、MOM、YOY 和分层回勾公式均写在 01—06 页；01—03 月度表同时列出整体盘与 Top100 的加权成交均价，并分别列出整体盘销量/销售额MOM和Top100销量/销售额MOM，年度表为整体、PP、高客单价、GENIMO及GENIMO PP范围提供销量YOY和销售额YOY。04 页按报告原顺序提供 GENIMO 整体市场份额和 GENIMO PP 市场份额两张公式表；90/91 页保留逐行输入和确定性去重明细，92 页提供可复核的聚合中间层。月度表明确拆开“BSR前100原始候选行数”和“BSR前100去重Listing数”，前者可直接与90页筛选结果核对，后者才进入Top100销量、销售额、均价和分层汇总。

数据覆盖必须先看 `93_数据校验`：当前主源读取到 73,811 行、去重后 63,555 个 Listing；2025.10—2026.07 的逐月行数和状态在 `92_聚合输入`、`01_整体市场月度`—`04_GENIMO品牌月度` 中均可回勾。主源包含 2022.06—2026.07 共 50 个月；月度表的表头行由读取器自动识别，避免因导出格式变化把有数据的子表误判为空。

`outputs/20260909-formula-market-analysis/户外地垫市场分析-公式版-20260909.xlsx` 是按旧版流程生成的历史文件。当前 SPEC 2.0 可回勾文件使用上面列出的 12 张证据和结果表；整体市场由 PP 和非 PP 组成，GENIMO 是整体市场中的品牌视角。当前文件已按修正后的自动表头识别逻辑重建，2025.10—2026.07 不再显示为“无数据”。

工作表映射如下：`00_总览与校验` 提供四部分导航和关键回勾；`01_整体市场月度`、`02_PP市场月度`、`03_高客单价市场月度` 分别提供整体盘、Top100、均价、加权成交均价和MOM；`04_GENIMO品牌月度`按报告 4.1/4.2 顺序提供 GENIMO 整体表现、整体份额、PP表现、PP份额和品牌MOM；`05_年度YOY` 提供整体、PP、高客单价、GENIMO和GENIMO PP的年度YOY；`06_BSR分层` 提供四个范围的头部/中部/尾部和五档MOM；`90_原始输入`、`91_去重明细`、`92_聚合输入`、`93_数据校验`、`94_规则说明` 记录可追溯输入、中间聚合、数学校验和操作边界。毛利率、FBA、Coupon只保留在原始输入证据页，不在前端分析表展示，也不参与利润计算。

上述公式、数量和结果均属于旧版工作簿的历史验证记录。按 SPEC 2.0 重建后，必须重新确认公式、分子、分母、Top100筛选顺序、四部分图表和文本结论，不能直接沿用旧缓存。

旧版90页按父ASIN优先、缺省时用ASIN形成统计键；组内按最小可解析BSR、字段完整度、源行ID确定代表行，同时保留原始行数和去重行数。该段只说明历史实现。新流程必须以 SPEC 2.0 的 BSR 先筛选、PP标题匹配、整体盘/Top100双层和四部分验收规则重新生成。

如需复现旧版文件，使用工作区 Node 运行 `src/build_formula_market_xlsx.mjs`；该脚本只读 `market.db` 与 `competitor_809440.db`，生成 90/91 输入及结果公式页，不修改 `data/raw/` 或写回数据库。它尚未完成 Phase 12 的原始主源和 BSR 先筛选流程对齐，不能据此宣称已生成 SPEC 2.0 交付物。
