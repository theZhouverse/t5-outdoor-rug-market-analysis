...
当前按 SPEC 2.0 生成的可回勾工作簿为 `outputs/20260911-spec2-market-analysis/户外地垫市场分析-SPEC2-可回勾版.xlsx`。工作表顺序为：`00_总览与校验`、`01_整体市场月度`、`02_PP市场月度`、`03_高客单价市场月度`、`04_GENIMO品牌月度`、`05_年度YOY`、`06_BSR分层`、`90_原始输入`、`91_去重明细`、`92_聚合输入`、`93_数据校验`、`94_规则说明`。所有展示指标的 SUMIFS、均价、成交均价、MOM、YOY 和分层回勾公式均写在 01—06 页；月度表只保留销量MOM和销售额MOM，年度表为整体、PP、高客单价和 GENIMO 四个范围分别提供销量YOY和销售额YOY；90/91 页保留逐行输入和确定性去重明细，92 页提供可复核的聚合中间层。月度表明确拆开“BSR前100原始候选行数”和“BSR前100去重Listing数”，前者可直接与90页筛选结果核对，后者才进入Top100销量、销售额、均价和分层汇总。
...
工作表映射如下：`00_总览与校验` 提供四部分导航和关键回勾；`01_整体市场月度`、`02_PP市场月度`、`03_高客单价市场月度`、`04_GENIMO品牌月度` 分别提供月度整体盘、Top100、均价、MOM和品牌份额；`05_年度YOY` 提供整体盘与Top100年度YOY；`06_BSR分层` 提供四个范围的头部/中部/尾部和五档MOM；`90_原始输入`、`91_去重明细`、`92_聚合输入`、`93_数据校验`、`94_规则说明` 记录可追溯输入、中间聚合、数学校验和操作边界。毛利率、FBA、Coupon只保留在原始输入证据页，不在前端分析表展示，也不参与利润计算。
