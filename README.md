...
> **当前生效规范**：`docs/SPEC.md` 2.0（2026-09-10）。本版本把业务交付固定为整体市场、PP市场、非PP市场、GENIMO品牌四部分；以原始卖家精灵工作簿为主源，先解析并标记小类BSR 1—100（包含100），再按标题中的完整单词 `plastic` 划分PP和非PP。数据库、JSON、Markdown、公式版XLSX以及本次重建前的HTML均属于历史产物；当前 HTML 由 src/build_spec2_html.mjs 直接从原始主源生成，其他交付物仍需按 Phase 12 逐项重建后才能作为新流程验收文件。
