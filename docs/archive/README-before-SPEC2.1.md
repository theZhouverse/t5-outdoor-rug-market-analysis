...
数据覆盖必须先看 `93_数据校验`：当前主源读取到 73,810 行、去重后 63,554 个 Listing；2025.10—2026.07 的逐月行数和状态在 `92_聚合输入`、`01_整体市场月度`—`04_GENIMO品牌月度` 中均可回勾。主源包含 2022.06—2026.07 共 50 个月；月度表的表头行由读取器自动识别，避免因导出格式变化把有数据的子表误判为空。
