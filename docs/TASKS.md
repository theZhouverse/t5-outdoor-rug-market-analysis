...
- [x] Task 11.1: 生成单一结果 XLSX，并将计算公式放入结果页
  - 内容: 新增生成器 `src/build_formula_market_xlsx.mjs` 与 `outputs/20260909-formula-market-analysis/户外地垫市场分析-公式版-20260909.xlsx`；结果页覆盖整体月度、分类、BSR Top100、头中尾、GENIMO整体份额、核心汇总、决策提示和校验；90页保留明细值、91页保留同规则中间聚合、92页登记数据操作与限制
  - 验证: 12 张工作表、31,874 条明细、399 条中间聚合、3,625 个公式；整体=PP+高客单非PP的销量/销售额差额均为0；Top100最大行数100；公式错误扫描0项；概览预览已渲染
- [ ] Task 11.2: 获取改名后的 GitHub 用户名/仓库完整地址并更新远端与发布链接
  - 内容: 当前 `origin` 仍指向旧地址，旧 Pages 地址已返回 Site not found；拿到新地址后再更新 README/SPEC/HTML 引用、推送新分支并做线上 HTTP 200 验证
