...
- [x] 22.2 将BSR键改为月份+BSR值，忽略ASIN，对同月同BSR的Q/T直接取平均，再汇总三档。
- [x] 22.3 将各业务表统一为固定九列，父体数量不进入业务表，MOM/YOY均按比较倍数写入，补充选用字段契约。
- [x] 22.4 改造构建程序、Excel公式、独立审计和HTML，重新生成全部交付并核验Q/T主汇总、月份+BSR值平均、MOM/YOY不减1和固定列；46,780候选行、46,854条BSR观察记录、1,334个父体月份组，审计PASS。
