...
## Phase 14: 2026-09-14 BSR主池顺序纠偏（当前优先）

- [x] 14.1 更新SPEC、README、TASKS、REVIEWS、人工复核说明和审计摘要：明确先筛BSR 1—100（含100），再按父ASIN去重，之后拆PP/高客单价/GENIMO。
- [x] 14.2 重写统一主源模型：删除未经BSR筛选的整体盘分支，使用BSR候选父ASIN主池作为四部分唯一统计范围；代表标题/品牌分类发生在筛选去重之后。
- [x] 14.3 重建XLSX公式链、HTML、JSON、Markdown和构建清单，确保PP+高客单价逐月回加整体，GENIMO来自同一主池。
- [x] 14.4 重新执行2025.09、2026.07、全部公式、缺失状态、分层、进留退、规划和跨文件验收，并记录MOM最终分母月份；本地独立源审计、公式缓存审计和输入扰动均通过，桌面Excel整簿重算仍未执行。
