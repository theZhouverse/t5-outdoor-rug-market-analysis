...
- [x] Task 11.6: 生成完整管理周报结果文件并同步来源登记
  - 内容: 13 页同时列示计划部 BI 全类目、计划部 BSR Top100 和 market.db 父体快照三条基准；15 页保留计划部实际/目标数据并对 `#DIV/0!` 等错误输入留空；93 页登记参考表头、公式来源和实际数据操作。
  - 验证: 生成脚本成功；工作簿 20 页、9,479 个公式、0 个缓存公式错误；核心回勾和缺失数据校验保持原结果。
