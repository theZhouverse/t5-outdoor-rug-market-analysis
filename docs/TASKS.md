...
## Phase 18: 2026-09-23 SPEC 3.3 父体平均口径（当前）

- [x] 18.1 将父体Q列“月销量”和T列“月销售额($)”改为同月同父体有效值算术平均，并明确空值、零值、覆盖率和`AVERAGE_CONFLICT`规则。
- [x] 18.2 将BSR改为同一父体月份内对应有效排名的平均，保留最小值/最大值作诊断，不把最小值作为代表。
- [x] 18.3 同步SPEC、流程文档、README和REVIEWS，说明3.1/3.2旧输出不能作为3.3验收结果。
- [ ] 18.4 改造构建程序、Excel公式、独立审计和HTML，重新生成全部交付并核验Q/T平均与BSR平均。

Phase 18.4完成前，上一版Excel、HTML、JSON和GitHub Pages只作历史记录。
