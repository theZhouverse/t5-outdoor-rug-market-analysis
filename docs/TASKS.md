...
本轮已先完成 `src/build_spec2_html.mjs` 与当前 HTML 的 SPEC 2.0 对齐：HTML 直接读取原始主源、覆盖 50 个月、保留整体盘与 Top100 双层、四部分和原始字段覆盖说明；JSON、Markdown、结果 XLSX 尚未在本轮重建，Task 12.4 仍未完成。

- [ ] Task 12.1: 按原始主源重新建立四部分数据处理顺序
  - 内容：以 `data/raw/地垫-卖家精灵市场数据.xlsx` 为主源；读取月度明细后先解析并标记小类BSR 1—100（包含100），再按商品标题完整单词 `plastic` 划分PP和非PP；保留整体盘与Top100两个统计层。
  - 验证：源文件指纹、表头行、BSR有效/无效计数、100名包含检查、PP/非PP互补检查均有可复核记录。
- [ ] Task 12.2: 统一四个业务部分的分析框架和参考表头
  - 内容：整体市场、PP、非PP、GENIMO四部分均提供销量、销售额、均价、月度同比、月度环比、年度/同周期同比、Top100、头中尾、五档和文字分析；GENIMO增加整体份额、PP份额、BSR表现和2027规划。
  - 验证：每个部分的字段、图表、时间基准和分母与 SPEC 2.0 一一对应，参考工作簿只作为结构/公式参考。
- [ ] Task 12.3: 重建图表、原始经营字段和策略输出
  - 内容：补齐销量/销售额柱状图、均价趋势图、同比/环比折线图、五档BSR环比图；只展示主源已有的价格、毛利率、FBA、Coupon等字段，不计算利润或补充缺失的成本、广告、库存字段；策略必须有数据证据、动作和触发条件。
...
- [ ] Task 12.4: 用同一批次重新生成并完成验收
  ...
  - 验证：四部分、整体=PP+非PP、Top100上限、五档合计、GENIMO分母、同比/环比基准、缺失状态、禁止利润推算和跨文件一致性全部通过。
