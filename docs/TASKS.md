...
## Phase 19: 2026-09-23 SPEC 3.4 BSR独立支线（当前）

- [x] 19.1 保留父体Q列“月销量”和T列“月销售额($)”的同月同父体有效值算术平均。
- [x] 19.2 将BSR改为候选筛选和独立分层支线，禁止行级/父体BSR平均回写92页。
- [x] 19.3 同步SPEC、流程文档、README和REVIEWS，说明分层支线可独立出现同一父体且不强制回加整体。
- [ ] 19.4 改造构建程序、Excel公式、独立审计和HTML，重新生成全部交付并核验Q/T主汇总与BSR分层支线。

Phase 19.4完成前，上一版Excel、HTML、JSON和GitHub Pages只作历史记录。
