...
## Phase 20: 2026-09-23 SPEC 3.5 BSR平均三档与字段契约（当前）

- [x] 20.1 保留父体Q列“月销量”和T列“月销售额($)”的同月同父体有效值算术平均。
- [x] 20.2 在独立BSR支线内对同月同父ASIN的有效排名取平均，只使用头/中/尾三档，不把BSR平均写入Q/T公式。
- [x] 20.3 补充A—Z源字段、核心指标、诊断字段和只读字段契约，说明R/S源增长率不直接使用。
- [ ] 20.4 改造构建程序、Excel公式、独立审计和HTML，重新生成全部交付并核验Q/T主汇总、BSR三档回加和字段覆盖。

Phase 20.4完成前，上一版Excel、HTML、JSON和GitHub Pages只作历史记录。
