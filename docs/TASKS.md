...
- [x] Task 11.1: 按原始需求逐表对应重建单一结果 XLSX
  - 内容: `src/build_formula_market_xlsx.mjs` 生成一份可交付工作簿；整体市场=PP+非PP，高客单非PP是排除PP后的补集，GENIMO只作为品牌视角。01/02/03对应整体市场，04/05/06对应PP，07/08/09对应非PP，10/11/12对应GENIMO品牌份额/进退层/2027规划，90/91/92/93分别为输入、聚合、校验、规则。
  - 验证: 17 张工作表、63,130 条明细、1,763 条聚合、8,048 个公式；粗分层1-20/21-50/51-100与细分层1-5/6-10/11-20/21-50/51-100齐全；00页有公式驱动的整体/PP/非PP/GENIMO趋势判断；2026H1整体销量=PP+非PP且销量/销售额差额均为0；Top100最大行数100；XLSX回读无公式错误缓存。
- [x] Task 11.2: 同步最新比较口径、去重边界和交付说明
  - 内容: 结果页同时给出MOM（当前月 vs 上一自然月）和YOY（当前月 vs 去年同月）；90页采用父ASIN优先的确定性代表记录并保留原始/去重行数；GENIMO主分母为整体市场。
  - 验证: `docs/SPEC.md`、`docs/REVIEWS.md`、`README.md` 已记录17页映射、公式数量、核心回勾、缺失状态和数据操作边界。
