...
for scope,_ in scopes:
    for month in months:
        ...
        for level in levels:
            ...
            aggregate[(scope,month,level)]=metrics([r for r in base if in_level(r,level)])
def prev_month(m):
...
# raw input
s=ws['90_原始输入']; rh=['月份','源行号','ASIN','SKU','品牌','商品标题','父ASIN','小类BSR','月销量','月销售额($)','价格($)','FBA($)','毛利率','Coupon','PP标记','GENIMO标记','BSR Top100标记']; setup(s,'原始输入（逐行保留）· 来源：data/raw/地垫-卖家精灵市场数据.xlsx',16,'原始工作簿字段逐行读取；PP只依据商品标题完整单词 plastic；空值保留为空，未补零。'); headers(s,3,rh)
for i,r in enumerate(raw,4):
    vals=[r.get('month'),r.get('sourceRow'),r.get('asin'),r.get('sku'),r.get('brand'),r.get('title'),r.get('parent'),r.get('rank'),r.get('sales'),r.get('revenue'),r.get('price'),r.get('fba'),r.get('margin'),r.get('coupon'),1 if r.get('plastic') else 0,1 if r.get('genimo') else 0,1 if finite(r.get('rank')) and 1<=r['rank']<=100 else 0]
    for c,v in enumerate(vals): s.write(i,c,'' if v is None else v,money_fmt if c in (9,10,11) else num_fmt if c in (7,8) else text_fmt)
s.freeze_panes(4,6); s.autofilter(3,0,len(raw)+3,16); s.set_column(0,0,11); s.set_column(1,4,16); s.set_column(5,5,60); s.set_column(6,16,14)
# dedup detail
...
# aggregate helper
s=ws['92_聚合输入']; ah=['月份','范围键','范围','层级','Listing数','销量','销售额($)','价格合计($)','价格有效数','配对销量','配对销售额($)','销量有效数','销售额有效数','数据状态']; setup(s,'聚合输入（由 91_去重明细确定性生成）',13,'本页由去重明细按月、范围和层级生成；01—06 展示页公式只引用本页。MOM按去年同月，YOY按上一年度；空子表标记为无数据，不按 0 伪造。'); headers(s,3,ah)
agg=[]
for scope,label in scopes:
    for m in months:
        for level in levels:
            z=aggregate[(scope,m,level)]; agg.append([m,scope,label,level,z['count'],z['sales'],z['revenue'],z['price_sum'],z['price_count'],z['paired_sales'],z['paired_revenue'],z['sales_valid'],z['revenue_valid'],data_status.get(m,'无数据')])
...
s.freeze_panes(4,0); s.autofilter(3,0,len(agg)+3,13); s.set_column(0,0,11); s.set_column(1,3,15); s.set_column(4,13,16); agg_last=len(agg)+4
AR="'92_聚合输入'!$A$5:$A$%d"%agg_last; BR="'92_聚合输入'!$B$5:$B$%d"%agg_last; DR="'92_聚合输入'!$D$5:$D$%d"%agg_last; SR="'92_聚合输入'!$N$5:$N$%d"%agg_last
AC={'count':'E','sales':'F','revenue':'G','price_sum':'H','price_count':'I','paired_sales':'J','paired_revenue':'K','sales_valid':'L','revenue_valid':'M'}
...
# monthly pages
mn={'overall':'01_整体市场月度','pp':'02_PP市场月度','nonpp':'03_高客单价市场月度','genimo':'04_GENIMO品牌月度'}; mh=['月份','整体/范围Listing','整体/范围销量','整体/范围销售额($)','平均标价($)','加权成交均价($)','销量有效数','销售额有效数','价格有效数','销量MOM','销售额MOM','Top100 Listing','Top100销量','Top100销售额($)','Top100平均标价($)','数据状态']; mrow={m:i+5 for i,m in enumerate(months)}
for scope,label in scopes:
    s=ws[mn[scope]]; setup(s,label+'·月度数据（公式）',15,'销量、销售额、平均标价和加权成交均价由 92_聚合输入公式汇总；MOM=去年同月，YOY=上一年度。Top100为BSR 1—100（含100）。'); headers(s,3,mh); s.freeze_panes(4,0); s.set_column(0,0,12); s.set_column(1,15,16)
    for i,m in enumerate(months,4):
        ...
        fm=[fnum,fnum,fnum,fmoney,fmoney,fnum,fnum,fnum,fpct,fpct,fnum,fnum,fnum,fmoney];
        if data_status.get(m)=='无数据': vals=['']*14
