...
export function writeReports(d){
 ...
 for(const [index,scope] of ['overall','pp','nonpp','genimo'].entries()){
  ...
  for(const [title,rows] of [['BSR候选行月度汇总',cat.monthly],['BSR候选行月度回勾',cat.topMonthly]]) md+='\n### '+title+'\n\n'+mdTable(['月份','候选行月次','销量','金额($)','标价($)','配对均价($)','销量MOM','金额MOM','去年同月候选行月次','可比性'],rows.map(r=>[r.month,fmt(r.count),fmt(r.sales),fmt(r.revenue,2),fmt(r.avgPrice,2),fmt(r.pairedAsp,2),fmtPct(r.momSales),fmtPct(r.momRevenue),fmt(r.comparisonCount),r.quality]))+'\n';
